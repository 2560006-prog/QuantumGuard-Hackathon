...
if (id === 'credit') {
  setTimeout(() => { if ((window as any).loadCreditScore) (window as any).loadCreditScore(); }, 100);
}
  if (id === 'status') {
    updateValidationStatusPage();
  }
      ...
      <div className="f-main">
        ...
        {activePage === 'documents' && (
          <div className="f-page active">
            <div className="f-topbar">
              <div>
                <div className="f-page-title">📄 Documents</div>
                <div className="f-page-sub">Upload and manage your verification documents</div>
              </div>
              <div className="f-topbar-right">
                <div style={{ fontSize: '12px', fontWeight: 700, padding: '5px 14px', borderRadius: '20px', background: docs.length >= 6 ? '#e8f5e9' : '#fff3e0', color: docs.length >= 6 ? '#2e7d32' : '#e65100', border: `1px solid ${docs.length >= 6 ? '#c8e6c9' : '#ffe0b2'}` }}>
            ...
            <div className="f-content">
              {!p ? (
                ...
                <div className="f-card" style={{ marginBottom: '16px' }}>
                  <div className="f-card-title">📤 Upload New Document</div>
                  <div style={{ display: 'flex', gap: '10px', alignItems: 'flex-end', flexWrap: 'wrap' }}>
                    <div style={{ flex: 1, minWidth: '180px' }}>
                      <label style={{ display: 'block', fontSize: '11px', fontWeight: 700, color: '#6b7280', textTransform: 'uppercase', letterSpacing: '.05em', marginBottom: '6px' }}>Document Type</label>
                      <select className="form-input" value={docType} onChange={e => setDocType(e.target.value)} style={{ height: '40px', width: '100%', borderRadius: '8px', border: '1.5px solid var(--border)', padding: '0 10px', fontSize: '13px', fontFamily: 'var(--font-body)', background: 'white', cursor: 'pointer' }}>
                        {DOC_TYPES.map(dt => <option key={dt.key} value={dt.key}>{dt.icon} {dt.label}</option>)}
                      </select>
                    </div>
                    <label style={{ background: 'linear-gradient(135deg,#4CAF50,#2e7d32)', color: 'white', padding: '10px 20px', borderRadius: '9px', cursor: uploading ? 'not-allowed' : 'pointer', fontSize: '13px', fontWeight: 700, opacity: uploading ? .6 : 1, whiteSpace: 'nowrap', display: 'flex', alignItems: 'center', gap: '6px' }}>
                      {uploading ? '⏳ Uploading...' : '📎 Choose File'}
                      <input type="file" accept="image/*,.pdf,.doc,.docx" style={{ display: 'none' }} disabled={uploading} onChange={e => { const f = e.target.files?.[0]; if (f) uploadDocument(f); e.target.value = ''; }} />
                    </label>
                  </div>
                </div>

                {/* Documents list — styled like screenshot */}
                <div className="f-card">
                  <div className="f-card-title" style={{ display: 'flex', alignItems: 'center', justifyContent: 'space-between', marginBottom: '0' }}>
                  ...
                  <div style={{ marginTop: '14px' }}>
                    {docs.length === 0 ? (
                      <div style={{ textAlign: 'center', padding: '40px 0', color: '#9ca3af' }}>
                        ...
                        <div style={{ fontSize: '14px', fontWeight: 600 }}>No documents uploaded yet</div>
                        <div style={{ fontSize: '12px', marginTop: '4px' }}>Use the upload button above to add documents</div>
                      </div>
                    ) : (
                      docs.map((doc: any) => {
                        const typeLabels: Record<string,string> = {
                          identity: 'Identity Proof', land: 'Land Document', bank: 'Bank Document',
                          crop: 'Crop Document', other: 'Other', aadhaar: 'Aadhaar',
                          photo: 'Farmer Photo', income: 'Income Certificate', landphoto: 'Land Photo',
                        };
                        const typeIcons: Record<string,string> = {
                          identity: '🪪', land: '🗺️', bank: '🏦', crop: '🌾', other: '📄',
                          aadhaar: '🪪', photo: '🧑‍🌾', income: '📃', landphoto: '🌾',
                        };
                        const sizeKB = doc.file_size ? (doc.file_size / 1024).toFixed(1) + ' KB' : '—';
                        const dateStr = doc.created_at ? new Date(doc.created_at).toLocaleDateString('en-IN', { day: 'numeric', month: 'short', year: 'numeric' }) : '—';
                        return (
                          <div key={doc.id} style={{ display: 'flex', alignItems: 'center', gap: '14px', padding: '14px 12px', borderRadius: '10px', border: '1px solid #e8f0e8', background: '#f8fdf8', marginBottom: '8px', transition: '.15s' }}
                            onMouseEnter={e => (e.currentTarget.style.borderColor = '#4CAF50')}
                            onMouseLeave={e => (e.currentTarget.style.borderColor = '#e8f0e8')}>
                            {/* Doc icon */}
                            <div style={{ width: '42px', height: '42px', background: '#e8f5e9', borderRadius: '10px', display: 'flex', alignItems: 'center', justifyContent: 'center', fontSize: '20px', flexShrink: 0 }}>
                              {typeIcons[doc.document_type] || '📄'}
                            </div>
                            {/* Doc info */}
                            <div style={{ flex: 1, minWidth: 0 }}>
                              <div style={{ fontSize: '13.5px', fontWeight: 700, color: '#1a1a1a', whiteSpace: 'nowrap', overflow: 'hidden', textOverflow: 'ellipsis' }}>
                                {doc.document_name}
                              </div>
                              <div style={{ fontSize: '11px', color: '#6b7280', marginTop: '3px', display: 'flex', alignItems: 'center', gap: '6px', flexWrap: 'wrap' }}>
                                <span style={{ background: '#e8f5e9', color: '#2e7d32', padding: '1px 7px', borderRadius: '10px', fontWeight: 600 }}>
                            ...
                            <div style={{ display: 'flex', gap: '6px', flexShrink: 0 }}>
                              <a href={doc.file_url} target="_blank" rel="noreferrer"
                                style={{ display: 'flex', alignItems: 'center', gap: '5px', background: '#e8f5e9', color: '#2e7d32', border: '1px solid #c8e6c9', borderRadius: '7px', padding: '6px 12px', fontSize: '12px', fontWeight: 700, textDecoration: 'none', cursor: 'pointer' }}>
                                👁️ View
                              </a>
                              <button onClick={() => deleteDoc(doc)}
                                style={{ display: 'flex', alignItems: 'center', background: '#fce4ec', color: '#c62828', border: '1px solid #f8bbd0', borderRadius: '7px', padding: '6px 10px', fontSize: '12px', fontWeight: 700, cursor: 'pointer' }}>
                                🗑️
                              </button>
                            </div>
                          </div>
                ...
                <div className="f-card" style={{ marginTop: '14px' }}>
                  <div className="f-card-title">✅ Required Documents Checklist</div>
                  <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: '8px' }}>
                    {DOC_TYPES.map(dt => {
                      const uploaded = docs.filter((d: any) => d.document_type === dt.key || d.document_type === dt.key);
                      const isUploaded = uploaded.length > 0;
                      return (
                        <div key={dt.key} style={{ display: 'flex', alignItems: 'center', gap: '10px', padding: '10px 12px', borderRadius: '9px', background: isUploaded ? '#f0fdf4' : '#fafafa', border: `1px solid ${isUploaded ? '#bbf7d0' : '#e0e0e0'}` }}>
                          <div style={{ width: '28px', height: '28px', borderRadius: '50%', background: isUploaded ? '#22c55e' : '#e0e0e0', display: 'flex', alignItems: 'center', justifyContent: 'center', fontSize: '13px', flexShrink: 0 }}>
                            {isUploaded ? '✓' : dt.icon}
                          </div>
                          <div style={{ flex: 1, minWidth: 0 }}>
                            <div style={{ fontSize: '12px', fontWeight: 700, color: isUploaded ? '#15803d' : '#374151' }}>{dt.label}</div>
                            <div style={{ fontSize: '10px', color: isUploaded ? '#16a34a' : '#9ca3af', marginTop: '1px' }}>
                              {isUploaded ? `${uploaded.length} file(s) uploaded` : 'Not uploaded'}
                            </div>
                          </div>
                          {!isUploaded && (
                            <label style={{ background: '#f3f4f6', border: '1px dashed #d1d5db', borderRadius: '6px', padding: '4px 8px', fontSize: '10px', fontWeight: 600, color: '#6b7280', cursor: 'pointer', flexShrink: 0 }}>
                              + Upload
                              <input type="file" accept="image/*,.pdf" style={{ display: 'none' }}
                                onChange={e => { const f = e.target.files?.[0]; if (f) { setDocType(dt.key); uploadDocument(f); } e.target.value = ''; }} />
                            </label>
                          )}
                        </div>
                      );
                ...
                <div style={{ padding: '10px 14px', background: '#f0fdf4', borderRadius: '10px', border: '1px solid #bbf7d0', fontSize: '12px', color: '#6b7280', marginTop: '14px' }}>
                  🔗 All documents are stored securely in <strong style={{ color: '#2e7d32' }}>Supabase Storage</strong>. Click <strong>View</strong> on any document to open it.
                </div>
              </>)}
            </div>
          </div>
        ...
        {activePage === 'credit' && (
          <div id="credit-score-container">
            <div style={{textAlign:'center',padding:'40px',color:'#6b7280'}}>
              <div style={{fontSize:'48px',marginBottom:'12px'}}>💳</div>
              <div style={{fontSize:'14px',fontWeight:'600'}}>Click below to calculate your credit score</div>
              <button onClick={()=>(window as any).loadCreditScore?.()} style={{marginTop:'16px',padding:'10px 24px',background:'linear-gradient(135deg,#16a34a,#15803d)',color:'white',border:'none',borderRadius:'9px',fontSize:'13px',fontWeight:700,cursor:'pointer',fontFamily:'Nunito,sans-serif'}}>
              ...
              <div style={{ display: 'grid', gridTemplateColumns: 'auto 1fr', gap: '20px', alignItems: 'start' }}>
                <div className="qr-card-wrap" style={{ maxWidth: '280px' }}>
                  <div className="qr-header">
                    ...
                    <div className="qr-header-name">QuantumGuard</div>
                  </div>
                ...
                <div style={{ display: 'flex', flexDirection: 'column', gap: '14px' }}>
                  ...
                  <div className="f-card">
                    ...
                    <div className="qr-actions" style={{ justifyContent: 'flex-start' }}>
                      ...
                      <button className="qr-action-btn share" onClick={() => { navigator.clipboard.writeText(`QuantumGuard ID: ${farmerId} | ${user?.full_name}`); toast.success('Identity link copied!'); }}>↗ Share QR</button>
